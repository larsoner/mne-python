...
def _magnetic_dipole_resid_jac(x, B, coils, whitener, too_close):
    ...
    if not np.isfinite(fwd).all():
        # too_close != "raise" and we landed inside a coil; report the worst possible
        # residual with a flat Jacobian so the fit stops here (with a GOF of ~0) rather
        # than erroring out on non-finite values
        return B.copy(), np.zeros((len(B), 3))
...
def _fit_magnetic_dipole(B_orig, x0, too_close, whitener, coils, guesses):
    ...
    # x0 is the previous time point's coil position (or a better guess-grid point).
