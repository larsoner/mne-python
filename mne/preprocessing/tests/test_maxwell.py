...
def test_movement_compensation_smooth():
    ...
    # MC increases noise
    _assert_shielding(raw_sss, power, 0.25, upper=0.26)
    ...
    _assert_shielding(raw_sss, power, 0.26, upper=0.27)
...
def test_st_overlap():
    ...
    assert _compute_rank_int(raw_tsss, proj=False) == 140
    _assert_shielding(raw_tsss, power, 35.6, upper=35.7)
...
def _assert_shielding(raw_sss, erm_power, shielding_factor, meg='mag',
    ...
    if upper is not None:
        assert factor <= upper, ('Shielding factor %0.3f > %0.3f'
                                 % (factor, upper))
