...
def _fwd_eeg_fit_berg_scherg(m, nterms, nfit):
    ...
    # layers, not the absolute head radius or origin, so cache on those: the
    # COBYLA minimization dominates make_sphere_model and the same (usually
    # default) model is fit over and over. Using the exact relative-radius
    # ratio also keeps scipy's COBYLA out of a near-degenerate regime it hits
    # when the ratio is instead formed from rounded absolute radii, where it
    # fails to converge and burns its entire function-evaluation budget.
...
def _fit_berg_scherg_cached(rel_rads, sigmas, nterms, nfit):
    ...
    # rhoend (final radius) sets the resolution of mu. The equivalent dipoles
    # sit at radii proportional to mu (order unity) while the fit's residual
    # variance is ~1e-4, so resolving mu below ~1e-4 gains nothing and, with
    # scipy's PRIMA-based COBYLA, can prevent convergence within maxfun in
    # near-degenerate cases (it then exhausts its whole evaluation budget).
    mu_0 = np.zeros(3)
