...
from .transforms import (transform_surface_to, _pol_to_cart, _cart_to_sph,
                         ...
                         apply_volume_registration, _fit_matched_points,
...
def _scale_helmet_to_sensors(system, surf, info):
    ...
    for key, f_ in ch_pos_from.items():
        t_ = [ch['loc'][:3] for ch in info['chs']
              if ch['ch_name'].startswith(key)]
        if not len(t_):
            continue
        fro.append(f_)
    ...
    logger.info(
        f'    Deforming to match info using {len(fro)} matched points:')
    ...
    logger.info(f'    2. Nonlinear displacement: '
                f'mean={mu:0.1f}, max={mx:0.1f} mm')
    surf['rr'] = new_rr
    complete_surface_info(surf, copy=False, verbose=False)
    return surf
